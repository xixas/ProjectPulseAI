...
def enhance_comment(client, comment_text, comment_type, model, temperature, max_tokens):
...
""" + comment_text,
    ...
    prompt = prompts.get(comment_type, "normal")  # Retrieve the specific prompt based on the comment type
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": "Don't add any extra notes or comments from your side."},
...
def main():
    parser = argparse.ArgumentParser(description="Enhance a comment using OpenAI.")
    parser.add_argument("input_file", help="Path to the JSON input file containing the comment.")
    ...
    enhanced_text = enhance_comment(
        client, data['text'], data['comment_type'], model, temperature, max_tokens
    )
